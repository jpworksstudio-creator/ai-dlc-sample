...
| NFR-U1-2 | BFF パターン: クライアント → `/api/chat` → Gemini。`AbortSignal` 30s | レイテンシ制御・キー秘匿を一元化 | サーバー経由で1ホップ増 |
...
| NFR-U1-6 | Server-only Secret（環境変数 `GEMINI_API_KEY`、Route Handler のみ） | クライアント露出防止 | ローカル開発に `.env.local` 必要 |
...
- **Trust Boundary**: ブラウザ（localStorage, UI）｜Next.js サーバー（Secret, 検証）｜Gemini API（一時推論のみ）
...
- **SSRF**: Gemini 公式エンドポイント固定。ユーザー入力から URL は生成しない
