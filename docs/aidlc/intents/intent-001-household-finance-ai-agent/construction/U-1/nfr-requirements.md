...
| NFR-U1-6 | Gemini API キーをクライアントに露出しない | ブラウザから `GEMINI_API_KEY` 参照不可。API Route のみ | KC-5, Security | L-4 |
...
- Secrets: `GEMINI_API_KEY` はサーバー環境変数のみ。`.env.local` を gitignore
- Error exposure: ユーザー向けは汎用メッセージ。詳細はサーバーログのみ（家計データを含めない）
- External API timeout: Gemini 呼び出し 30s（`AbortSignal`）
